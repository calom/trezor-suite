...
import type { DiscoveryAccount, SelectAccount, DiscoveryAccountType } from '@trezor/connect';

const setHeader = (payload: SelectAccount['payload']) => {
...
export const selectAccount = (payload: SelectAccount['payload']) => {
    if (!payload) return;
